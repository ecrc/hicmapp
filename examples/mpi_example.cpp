...
int main(int argc, char *argv[]) {
    ...
    int num_of_cpus = 1;
    int n_processes_rows = 1;
    int n_processes_cols = 2;
...
//        auto decomposer = TwoDimCyclicDecomposer(size, 1);
        ...
        // Dense Warmup
        {
            ...
            hicmapp::api::Hicmapp<double>::Gemm(a_dense, blas::Op::NoTrans, b_dense, blas::Op::NoTrans, c_dense,
...
//            std::ofstream fs = std::ofstream("DenseHicmaMatrix.txt." + std::to_string(id));
//            c_dense.Print(fs);
//            fs.close();
